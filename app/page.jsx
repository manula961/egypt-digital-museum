...
export default function Home(){
  ...
  const filtered=useMemo(()=>{
    ...
    return artifacts.filter(a=>(era==="All"||a.era===era)&&(!q||`${a.title} ${a.text} ${a.type} ${a.era}`.toLowerCase().includes(q)));
  },[query,era]);
  return <main id="top" className={night?"night":""}>
    ...
    <section id="collection" className="section" aria-labelledby="collection-title"><div className="sectionHead"><div><p className="eyebrow">THE COLLECTION</p><h2 id="collection-title">Objects that <em>speak</em>.</h2></div><p>Curated highlights from ancient Egyptian culture, architecture and writing.</p></div>
      ...
      {filtered.length?<div className="grid">{filtered.map((a,i)=><ArtifactCard key={a.id} artifact={a} index={i} onOpen={setSelected}/>)}</div>:<div className="empty"><h3>No objects found</h3><p>Try a different search or select another era.</p><button type="button" className="secondary" onClick={()=>{setQuery("");setEra("All")}}>Reset filters</button></div>}
    </section>
    ...
    <footer><a className="brand" href="#top"><span aria-hidden="true">𓂀</span><span><strong>EGYPT</strong><small>DIGITAL MUSEUM</small></span></a><p>Built for the CyberNex digital museum experience.</p></footer>
