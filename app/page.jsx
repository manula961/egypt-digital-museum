...
export default function Home(){
  ...
  return <main id="top" className={night?"night":""}>
    ...
    <section id="collection" className="section" aria-labelledby="collection-title"><div className="sectionHead"><div><p className="eyebrow">THE COLLECTION</p><h2 id="collection-title">Objects that <em>speak</em>.</h2></div><p>Curated highlights from ancient Egyptian culture, architecture and writing.</p></div>
      <div className="toolbar"><label className="search"><span>Search the collection</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search the collection…" /></label><div className="filters" role="group" aria-label="Filter by category"><button type="button" aria-pressed={category==="All"} className={category==="All"?"active":""} onClick={()=>setCategory("All")}>All categories</button>{categoryMap.map(([e])=><button type="button" key={e} aria-pressed={category===e} className={category===e?"active":""} onClick={()=>setCategory(e)}>{e}</button>)}</div><div className="filters" role="group" aria-label="Filter by era">{eras.map(e=><button type="button" key={e} className={era===e?"active":""} aria-pressed={era===e} onClick={()=>setEra(e)}>{e}</button>)}</div></div>
