...
export default function Navigation({ night, onToggleNight }) {
  ...
  return <nav className="nav" aria-label="Primary navigation">
    ...
    <button className="menuButton" type="button" aria-label={open?"Close navigation menu":"Open navigation menu"} aria-expanded={open} aria-controls="mobile-menu" onClick={()=>setOpen(v=>!v)}>{open?"Close":"Menu"}</button>
